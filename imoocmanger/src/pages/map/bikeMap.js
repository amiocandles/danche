...
export default class Order extends React.Component{
    ...
    renderMap=(res)=>{
        ...
        //绘制车辆行驶路线
        let routeList = [];
        list.forEach((item)=>{//item是字符串split分隔
            let p = item.split(",");
            //let point = new window.BMap.Point(p[0], p[1]);
            routeList.push(new window.BMap.Point(p[0], p[1]));
        })
        // 行驶路线
        var polyLine = new window.BMap.Polyline(routeList, {//polyLine折现
            strokeColor: "#ef4136",
            strokeWeight: 3,
            strokeOpacity: 1
        });
        ...
        serviceList.forEach((item) => {
            ...
            servicePointist.push(new window.BMap.Point(item.lon, item.lat));
        })
        // 画线
