...
class FilterForm extends React.Component{
    ...
    handleFilterSubmit = ()=>{
        let fieldsValue = this.props.form.getFieldsValue();//获取表单中所有的值
    ...
    initFormList=()=>{
        ...
        const formList = this.props.formList;//协助从外层获取formList
        const formItemList = [];
        if (formList && formList.length>0){
            formList.forEach((item,i)=>{
                ...
                if (item.type == '城市'){
                    const begin_time = <FormItem label="城市" key={field}>
                        {
